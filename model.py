        ...
        for i in range(self.layers):
            ...
            if self.training:  # 仅训练时更新边权重
                ...
                if torch.any(is_user_item):
                    u_emb = user_emb[u_idx[is_user_item] - 1]  # 减去padding_idx
                    i_emb = item_emb[v_idx[is_user_item] - num_users - 1]
                    uv_feat = torch.cat([u_emb, i_emb], dim=1)
                    edge_weights.append(self.adj_weight(uv_feat).squeeze())

                if torch.any(is_item_user):
                    i_emb = item_emb[u_idx[is_item_user] - num_users - 1]
                    u_emb = user_emb[v_idx[is_item_user] - 1]
                    vu_feat = torch.cat([i_emb, u_emb], dim=1)
                    edge_weights.append(self.adj_weight(vu_feat).squeeze())
